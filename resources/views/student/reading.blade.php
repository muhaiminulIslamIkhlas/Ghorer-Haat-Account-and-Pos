          ...
          <div class="col-md-9">
                    ...
                    <div class="card shadow p-3 mb-5 bg-white rounded">
                      ...
                      <div class="card ">
                        ...
                        <p class="p-2">
                        @foreach($data as $row)
                            @if($row->question_no == $ro->question_no)
                   
                            
                          <span>{{$row->question}} <select  class="rounded bg-primary text-white">
                            <option  value="{{$row->option_1}}">{{$row->option_1}}</option>
                            <option value="{{$row->option_2}}">{{$row->option_2}}</option>
                            <option value="{{$row->option_3}}">{{$row->option_3}}</option>
                            <option value="{{$row->option_4}}">{{$row->option_4}}</option>
                          </select></span>
                      ...
                      <br>
                        <div class="card m-2">
                          <p class="p-2">{{$row->question_sort_code}}</p>
                        </div>
                      <br>
                      <div class="card m-2">
                          <div class="card-header bg-primary text-white">Select Your Answer(Ans can be more than one)</div>
                          <div class="card-body">
                          @foreach($an as $ro)
                            @if($row->id == $ro->question_id)
                              <input type="checkbox" name="vehicle[]" value="{{$ro->ans}}"> {{$ro->ans}}<br>
                            @endif
                          @endforeach
